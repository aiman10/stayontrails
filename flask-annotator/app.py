...
def create_app() -> Flask:
    ...
    @app.get("/api/models/<model>/training/status")
    def training_status(model):
        from annotations import load_annotations
        from dataset_export import all_images_done
        ...
        loaded = load_annotations(model_dir, model)
        images = loaded["data"].get("images", [])
        ready, done_count, total = all_images_done(images)
        available, error = training_service.is_available()
        return {
            "ok": True,
            "ready": ready,
            ...
            "available": available,
            "error": error,
            "latest_run": training_service.latest_run(model),
        }

    @app.post("/api/models/<model>/training/start")
    def training_start(model):
        from annotations import load_annotations, save_annotations
        from dataset_export import all_images_done, compute_splits, write_dataset
        import training_service
        ...
        if not ready:
            return {
                "ok": False,
                "error": f"Not all images annotated ({done_count}/{total}).",
            }, 400

        available, err = training_service.is_available()
        if not available:
            return {"ok": False, "error": err}, 503

        body = request.get_json(silent=True) or {}
        epochs = int(body.get("epochs", 50))
        model_size = str(body.get("model_size", "yolov8n-seg"))

        # Persist the splits used for this run into annotations.json.
        files = [img["file"] for img in images]
        splits = compute_splits(files)
        data["splits"] = splits
        save_annotations(model_dir, data)

        # Stage YOLO dataset on disk.
        dataset_dir = training_service.FLASK_DIR / "_datasets" / model
        ...
        state = training_service.start_run(
            model=model, data_yaml=data_yaml, epochs=epochs, model_size=model_size,
        )
        return {"ok": True, "run": state}

    @app.get("/api/models/<model>/training/runs/<run_id>")
    def training_run_state(model, run_id):
        import training_service

        model = slugify(model)
        if not model:
            abort(400, description="Invalid model.")
        # run_id is generated by us as YYYYMMDD_HHMMSS — defend against traversal.
        if not run_id.replace("_", "").isdigit():
            abort(400, description="Invalid run id.")
        st = training_service.read_state(model, run_id)
        if st is None:
            abort(404, description="Run not found.")
        return {"ok": True, "run": st}

    @app.get("/api/models/<model>/training/runs/<run_id>/best.pt")
    def training_run_download(model, run_id):
        import training_service
        ...
        if not run_id.replace("_", "").isdigit():
            abort(400, description="Invalid run id.")
        rdir = training_service.run_dir(model, run_id)
        if not (rdir / "best.pt").is_file():
            abort(404, description="best.pt not found (training may not have finished).")
        return send_from_directory(str(rdir), "best.pt", as_attachment=True)
